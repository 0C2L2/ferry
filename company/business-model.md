...
## Billing status (implementation honesty)

Checkout, entitlements, and plan enforcement are **not built yet** — they ship with the accounts launch (see `MASTER-PLAN.md` M6). Until then the working Cloud Backup flow runs open: prices below are the plan, not a charge. Nothing here changes the two rules.
