...
import type { DriveInfo, UsbLayout } from "../types";
...
export function PartitionUsb({ drive, onDone }: Props) {
  ...
  async function run() {
    ...
    setStage("active");
    try {
      const layout = await api.prepareUsb(drive.drive_letter);
      setStage("done");
      onDone(layout);
    } catch (err) {
      setStage("error");
        ...
        <>
          <div className="bg-red-50 border border-red-200 text-red-900 rounded-lg p-4 mb-4 flex gap-2 text-sm">
            ...
            <p>
              ...
              {drive.model}) and set it up as a Ferry USB — a small boot partition plus a large
              storage partition. Nothing on your PC itself is touched, only this drive.
            </p>
          </div>
